...
export function Sidebar({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("hidden md:flex flex-col w-64 border-r border-white/10 bg-zinc-950 min-h-screen relative p-4", className)}>
      <div className="flex items-center gap-2 px-2 mb-8">
        <BrainCircuit className="w-6 h-6 text-indigo-500" />
        <span className="text-lg font-bold font-sans tracking-tight text-white">AI Startup Gen</span>
      </div>
      ...
      <nav className="flex-1 space-y-1">
        {navigation.map((item) => (
          ...
          >
            <item.icon className="mr-3 flex-shrink-0 w-5 h-5" aria-hidden="true" />
            {item.name}
          </NavLink>
        ))}
      </nav>
      ...
      <div className="mt-auto space-y-1 pt-4 border-t border-white/10">
        <a href="#" className="group flex items-center px-3 py-2 text-sm font-medium text-gray-400 rounded-lg hover:bg-white/5 hover:text-white transition-colors">
